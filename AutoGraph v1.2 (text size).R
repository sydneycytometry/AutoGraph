...
###################################################### 2. SETUP DATA ######################################################     
    ...
    ### 2.1. Setup data
        ...
        data <- read.csv("BrdU percent pos.csv")
        ...
        Colour_Order_Fill <- c("Black", "red") # number of distinct columns must be the same as the number of groups -- MUST MATCH ALPHABETICAL ORDER
        Colour_Order_Lines <- c("Black", "red") # "Black" will create an outline of each point. Can change to match the colours on the line above.
        ...
        Y_axis_label <- "Percent BrdU positive" # Y axis labl
    ...
    ### 2.2. Setup statistics
        ...
        Variance_test <- "kruskal.test" # "kruskal.test"   # can be "kruskal.test" (non-parametric) or "anova" (parametric)
        ...
        my_comparisons <- list(c("Mock_D7", "WNV_D7")) # comparison 3 etc
    ...
    ### 3.2. LOOP
        ...
        for (a in ColNames) {
            ...
            # assign max and min values
                ...
                max_y_value_p10 <- max_y_value*1.2
            ...
            p <- ggplot(data, aes(x=Xaxis, y=data[[a]]))+ #, fill=Species)) + # can use fill = Species -- makes coloured by Species, with block outline of point -- NEED FILL for violin plot to be filled
              ...
              # MORE THAN TWO GROUPS: pairwise comparison with overall anova/Kruskal-Wallis result
                    ...
                    stat_compare_means(method = Variance_test, label.y = max_y_value_p40, size = 4)+ # Add global Anova ("anova") or Kruskal-Wallis ("kruskal.test", default) p-value # an add label.y = 50 to specifiy position
              ...
              ## THEMES
                  theme(legend.position = "none", # can be "left" "right" "top" "bottom" "none
                        axis.text.x = element_text(colour="black",size=14,angle=45,hjust=1,vjust=1,face="bold"),
                        axis.text.y = element_text(colour="black",size=14,angle=0,hjust=1,vjust=0,face="bold"),  
                        axis.title.x = element_text(colour="black",size=14,angle=0,hjust=.5,vjust=0,face="bold"),
                        axis.title.y = element_text(colour="black",size=14,angle=90,hjust=.5,vjust=1,face="bold"),
                        plot.title = element_text(lineheight=.8, face="bold", hjust = 0, size = 18), # hjust = 0.5 to centre
            ...
            ## Save        
            ggsave(p, filename = paste0(plotname, ".pdf"), width = 3.6, height = 5)
